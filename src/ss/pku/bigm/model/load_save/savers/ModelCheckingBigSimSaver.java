package ss.pku.bigm.model.load_save.savers;
...
import ss.pku.logic.FormulaValidation;
import ss.pku.utils.PairRule;
import ss.pku.utils.RuleInOut;

public class ModelCheckingBigSimSaver extends Saver {
	...
	@Override
	public ModelCheckingBigSimSaver setModel(ModelObject model) {
...
//		write("{agentName:" + agentName + "}");
	 ...
	 * @author Kevin Chan 
	 */
	private boolean ifLTLValid(String s) {
		String[] ltlElementsArray = s.split(" ");
		List<String> ltlElements = Arrays.asList(ltlElementsArray);
		return false;
	}
	
	/**
	 * @author Kevin Chan input the LTL Formula 
	 */
	private void processFormula(String s) {
		try {			
			write(s);
			write("\n");
	...
	private void processSimulationSpec(SimulationSpec s)
		...
		// 检测规约合法性
		FormulaValidation formulaValidator = new FormulaValidation(s.getLTL_Formula());
		try {
			formulaValidator.isValid();
		} catch (ScriptException e) {
			e.printStackTrace();
			throw new SaveFailedException(e);
		}
		if (!formulaValidator.specialCheck()) {
			throw new SaveFailedException();			
		} else {
...
//		processNamedControls(s);
		...
		write("# Model Checking info \n");
		processFormula(s.getLTL_Formula());

		//!TODO Kevin Chan 20170402
		List<? extends BigraphAP> props = s.getPropositions();
		
		if (props.size() != 0) {
			write("# Proposition \n");
			for (BigraphAP ap: s.getPropositions()) {
				Bigraph b = ap.getBigraph();
				processModel(b);
			}
			write("\n");
		}
	 ...
	 * @author Kevin Chan
	 * 2017.3.16
	 * This method of forming the total proposition
	 */
	@SuppressWarnings("unused")
	private String formProp(SimulationSpec s) {

...
public class TestingAndAnalysisBasedOnConfigDialog extends TitleAreaDialog {
	private Text dmFile;
	private Button browseDM;
	private int BROWSE_DM_ID = 1000;// bigraph data model file browse
	private Button runBigDataGen;
	private int BIG_DATA_ID = 2000;// bigraph data model file run
	
	private Text efsmFile;
	private Button browseEFSM;
	private int BROWSE_EFSM_ID = 1001;// EFSM model file browse
	private Button runEFSMUnit;
	private int EFSM_UNIT_ID = 2001;// EFSM model file run
	...
	private Button runBigMC;
	private int BIGMC_ID = 2002;// run bigMC for bigM run

	private Text[] inputFiles = new Text[4];
	private Button[] browseFiles = new Button[4];
	private int[] BROWSE_FILE_ID = {4000, 4001, 4002, 4003};// run model interleaving 4browse btn
	private Button runModelInter;
	private int MODEL_INTER_ID = 2003;// run modle interleaving run
	
	private Button runAll;
	private int ALL_TEST_ID = 2004;// 最后的那个button
	
	int index = 0;
	
	private String systemPath = Platform.getBundle(BigMPlugin.PLUGIN_ID).getLocation().replace("initial@reference:file:", "");
	private String imageFile = systemPath + "resources/icons/run.gif";	
	...
	private String paths = systemPath + "resources/doc/paths_alldef.txt";
	private String defs = systemPath + "resources/doc/dpath.txt";
	private String bgm = systemPath + "resources/doc/decoction(new2).bgm";
	private String resDir = systemPath + "resources/doc/";	
	private String fileName = "AnalysisReport.xls";	
			
	
	public TestingAndAnalysisBasedOnConfigDialog(Shell shell) {
	...
	protected Control createDialogArea(Composite parent) {
		...
		runBigDataGen = createButton(bigDataGen, BIG_DATA_ID, "", true);
		runBigDataGen.setLayoutData(buttonGrid);
		runBigDataGen.setImage(new Image(null, imageFile));
		...
		runEFSMUnit = createButton(efsmUnit, EFSM_UNIT_ID, "", true);
		runEFSMUnit.setLayoutData(buttonGrid);
		runEFSMUnit.setImage(new Image(null, imageFile));
		...
		bigMCforBigM.setLayout(new GridLayout(4, false));
		...
		runBigMC = createButton(bigMCforBigM, BIGMC_ID, "", true);
		runBigMC.setLayoutData(buttonGrid);
		runBigMC.setImage(new Image(null, imageFile));
		...
		modelInterleaving.setLayout(new GridLayout(4, false));
		...
		runModelInter = createButton(modelInterleaving, MODEL_INTER_ID, "", true);
		runModelInter.setLayoutData(buttonGrid);
		runModelInter.setImage(new Image(null, imageFile));
	...
	public void buttonPressed(int buttonId){
		 if(buttonId == BROWSE_EFSM_ID){// 1001 EFSM model file
		 ...
		 } else if(buttonId == EFSM_UNIT_ID){
			 
			 runEFSMUnit.addSelectionListener(new SelectionAdapter(){
					public void widgetSelected(SelectionEvent e){
						//generate paths and data
					}
			 });
		 ...
		 } else if(buttonId == BIGMC_ID){
			 String pathset = "C:\\Users\\chenjing\\Desktop\\pathset.txt";
			 String defFile = "C:\\Users\\chenjing\\Desktop\\dpath.txt";
			 String bgmFile = files[0].getText();
			 org.bigraph.bigmcDF.BigMC.main(new String[]{"-D", "alldefs", pathset, defFile, bgmFile});
			 MessageDialog.openInformation(getParentShell(), "Path Set Completed", "Model Check Finished! Please check " + pathset + " for details.");
			 buttonPressed(OK);
		 } else if(buttonId == MODEL_INTER_ID){
			 try {
					Runtime.getRuntime().exec("cmd.exe   /C   startup.bat ",
												null, 
												new File("D:\\Program Files\\tomcat\\bin"));
												//new File("C:\\Program Files\\Apache Software Foundation\\Tomcat 6.0\\bin"));
					try {
						Thread.sleep(15000);
				...
				buttonPressed(OK);
		 } else if(buttonId == BIG_DATA_ID){
			 runBigDataGen.addSelectionListener(new SelectionAdapter(){
					public void widgetSelected(SelectionEvent e){
						//generate paths and data
					}
			 });
			 ...
			 org.bigraph.bigmcDF.BigMC.main(new String[]{"-D", "alldefs", paths, defs, bgm});
	...
	public void buildWorkBook(String resDir, String fileName, String efsm, String paths, String defs){
		...
		EFSMModel efsmModel = efsmParser.parserXml(efsm);
		efsmModel.initStates();
		PathSet pathSet = new PathSet(efsmModel);
//		int basePathNum = pathSet.getPathList().size();
		DataGenerationByRandom dg = new DataGenerationByRandom();
		Random r = new Random();
		ArrayList<Integer> index = new ArrayList<Integer>();
		while(index.size() < 10){
			int pathIndex = r.nextInt(10);
			if(index.indexOf(pathIndex) != -1){
				index.add(pathIndex);
			}
		}
...
//			int basePathIndex = r.nextInt(basePathNum);
			ArrayList<State> basePath = pathSet.getPathList().get(i);
			LinkedList<State> tPath = pathSet.createRandomPathWithCycleByBase(pathSet.getPathList().get(i));
			ArrayList<ArrayList<String>> varList = dg.dataGenerationForPath(efsmModel, pathSet, i, tPath);

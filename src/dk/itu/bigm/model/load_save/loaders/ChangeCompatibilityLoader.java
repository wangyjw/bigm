package dk.itu.bigm.model.load_save.loaders;

import static org.bigraph.model.loaders.BigmNamespaceConstants.CHANGE;
...
import dk.itu.bigm.model.load_save.BigMXMLUndecorator;
...
public final class ChangeCompatibilityLoader
	...
	public IChangeDescriptor changeDescriptorFromElement(org.w3c.dom.Node n) {
		...
		} else if (el.getNamespaceURI().equals(BIGM)) {
			if (el.getLocalName().equals("layout")) {
				String
					type =
						getAttributeNS(el, BIGM, "type"),
					name =
						getAttributeNS(el, BIGM, "name");
				Layoutable.Identifier l = getLayoutable(type, name);
				...
				if (l != null)
					cd = LayoutUtilities.changeLayoutDescriptor(l,
							...
							BigMXMLUndecorator.getRectangle(el));
			} else if (el.getLocalName().equals("fill")) {
				String
					colour =
						getAttributeNS(el, BIGM, "colour"),
					type =
						getAttributeNS(el, BIGM, "type"),
					name =
						getAttributeNS(el, BIGM, "name");
				...
				String
					comment =
						getAttributeNS(el, BIGM, "comment"),
					type =
						getAttributeNS(el, BIGM, "type"),
					name =
						getAttributeNS(el, BIGM, "name");
				...
				if (l != null)
					cd = ExtendedDataUtilities.changeCommentDescriptor(l,
							...
							comment);
			} else if (el.getLocalName().equals("nodeValue")) {
				//!TODO ÐÞ¸Ä nodeValue

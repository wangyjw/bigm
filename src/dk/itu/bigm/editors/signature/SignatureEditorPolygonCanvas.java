...
import dk.itu.bigm.model.Line;
import dk.itu.bigm.utilities.ui.ColorWrapper;
import dk.itu.bigm.utilities.ui.UI;

import static dk.itu.bigm.model.ControlUtilities.changeDistance;
import static dk.itu.bigm.model.ControlUtilities.changeSegment;
import static dk.itu.bigm.model.ControlUtilities.getDistance;
import static dk.itu.bigm.model.ControlUtilities.getSegment;
...
public class SignatureEditorPolygonCanvas extends Canvas implements
	...
	public static String portSort = null;
	...
	public IInputValidator getPortNameValidator(final PortSpec current) {
	...
	private void opAddPort(PortSpec port, String portName, String portSort, int segment, double distance) {
		ChangeGroup cg = new ChangeGroup();
		if (distance >= 1.0) {
			distance = 0.0;
			segment = (segment + 1) % getPointCount();
		}
		cg.add(getModel().changeAddPort(port, portName, portSort));
			...
			if (segment != -1) {
				UI.createMenuItem(m, 0, "Add &port", new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						...
						PortCreationDialog portDialog = new PortCreationDialog(getShell(), model);
						portDialog.open();
						if(portName != null && portSort != null){
							opAddPort(new PortSpec(),
									portName, portSort, newSegment, newDistance);
						}
					}
				});
			}

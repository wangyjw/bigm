...
public class ModelPropertySource implements IRedPropertySource {
	...
	protected void buildPropertyDescriptors() {
		...
		if (object instanceof Link) {
			 ...
			 */
			addPropertyDescriptor(new PropertyDescriptor(
					ExtendedDataUtilities.PORTINDEX, "Port Index"));
			// port index ends
		...
		//!TODO 增加属性值
		addPropertyDescriptor(new NullTextPropertyDescriptor(
					ExtendedDataUtilities.NODEVALUE, "Value"));
		//!TODO 新属性值
	...
	/**
	 * @author Kevin Chan
	 * @return if this name is valid for this object by formation rules 
	 */
	public boolean isNameValid(Identifier target, String name) {
	 ...
	 * @return if this name is valid for this object by formation rules 
	 */
	public boolean isNameValid(Layoutable l, String name) {
		if (l != null && l instanceof Node) {
			Node n = (Node) l;		
			Control c = n.getControl();
			String cName = c.getName();
			FormRules rules = c.getSignature().getFormRules();
			for (FormationRule rule: rules.getFormationRules()) {
				if (rule.getType().equals("Formation Rule for Asigning")
						&& rule.getSort2().equals(name)) {
					if (rule.getSort1() != null && (!rule.getSort1().equals(cName))) {
//						System.out.println("test_2017");
						return false;
					}
				}
			}
	...
	public Object getPropertyValue(Object id) {
		if (id.equals("Class")) {
			...
			} else if (ExtendedDataUtilities.NODEVALUE.equals(id)) {
				return ExtendedDataUtilities.getNodeValue(object);
			 ...
			 * @author Kevin Chan 
			 */
			} else if (ExtendedDataUtilities.PORTINDEX.equals(id)) {
				return ExtendedDataUtilities.getPortIndex((Port) object);
			// ends
	...
	public final void setPropertyValue(Object id, Object value) {
		setPropertyValueChange(id, value);
...
//				+ value).fillInStackTrace());
	...
	/**
	 * edited by Kevin Chan in checking the validation of object
	 */
	@Override
	public IChange setPropertyValueChange(Object id, Object newValue) {
		if (Layoutable.PROPERTY_NAME.equals(id)) {
			if (!isNameValid(getModel(), newValue.toString())) {
				//!TODO valid name
				MessageDialog.openWarning(null, "Wrong", "This node has target control");
				return null;
			} else {
				return new BoundDescriptor(getModel().getBigraph(),
						new NamedModelObject.ChangeNameDescriptor(getModel()
			...
			return ExtendedDataUtilities.changeNodeValue(getModel(), 
					(String) newValue);
		} else if (ExtendedDataUtilities.INNERSORT.equals(id)) {

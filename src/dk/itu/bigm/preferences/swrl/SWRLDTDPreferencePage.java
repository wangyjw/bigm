...
public class SWRLDTDPreferencePage
	...
	public SWRLDTDPreferencePage() {
		setPreferenceStore(BigMPlugin.getInstance().getPreferenceStore());
		setDescription("Built-in configuration for SWRL Date/Time/Duration");
	}
		
	@Override
	protected Control createContents(Composite parent) {
		keyTable = new Table(parent, SWT.MULTI|SWT.BORDER|SWT.FULL_SELECTION | SWT.CHECK);
		CommonFuncUtilities.drawTableWithCheck(parent, keyTable, LinkSortKey.keysForSWRLDTD, keySetDTD);
		
		CommonFuncUtilities.copyHashMapToHashMap(keySetDTD, beforeAction);
